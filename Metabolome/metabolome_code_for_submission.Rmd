...
if(!exists("mdm2HiLoLMME")){
    ...
    mdm2HiLoLMME <- mdm2HiLoLMME %>% mutate(sig0.05=ifelse(mypadj<pval_thresh & abs(log2fc) > 0.75,TRUE,FALSE))
    mdm2HiLoLMME <- mdm2HiLoLMME %>% mutate(sig0.1=ifelse(mypadj<0.1 & abs(log2fc) > 0.75,TRUE,FALSE))
}
...
write.csv(resMDM2HiLoLMME,"~/Desktop/temp.csv")
...
if(!exists("mdm2TreHiLoLMME")){
    ...
    mdm2TreHiLoLMME <- mdm2TreHiLoLMME %>% mutate(alpha=ifelse(mypadj<pval_thresh & abs(log2fc) > 0.75,0.05,0.025))
    mdm2TreHiLoLMME <- mdm2TreHiLoLMME %>% mutate(size=ifelse(mypadj<pval_thresh & abs(log2fc) > 0.75,4,2))
}
...
mdm2LoTreLMME = mdm2LoTreLMME %>%
    mutate(class=ifelse(mypadj<pval_thresh & abs(log2fc) > 0.75,
                        as.character(metabmeta$SUPER.PATHWAY[match(name,metabmeta$BIOCHEMICAL)]),
                        "Not Significant"))
p<-ggplot(data=mdm2LoTreLMME, aes(x=log2fc, y=-log10(myp), colour=class)) +
    geom_point(aes(size=size,text=name)) +
    theme_bw() +
    scale_color_manual(values = myPalette) +
...
## Treated vs Untreated, All Samples
...
if(!exists("mdm2TrevsUntre")){
  ...
  mdm2TrevsUntreLMME <- mdm2TrevsUntreLMME %>% mutate(alpha=ifelse(mypadj<pval_thresh & abs(log2fc) > 0.75,0.05,0.025))
  mdm2TrevsUntreLMME <- mdm2TrevsUntreLMME %>% mutate(size=ifelse(mypadj<pval_thresh & abs(log2fc) > 0.75,4,2))
}
...
metabs_of_interest<-unique(c(metabs_of_interest,as.matrix(mdm2Lo_Treated_vs_UntreatedSig$name)))
## metabs_of_interest<-as.matrix(mdm2Lo_Treated_vs_UntreatedSig$name)
...
sampmeta_filtered<-left_join(data.frame(SAMPLE.NAME=colnames(metabFiltered)),sample_table, by = "SAMPLE.NAME")
...
metab_metadata<-list(mdm2hi_filtered,
                 ...
                 left_join(data.frame(SAMPLE.NAME=colnames(metabFiltered)),sample_table,by="SAMPLE.NAME") %>% dplyr::select(MANIFEST.ID)
)
