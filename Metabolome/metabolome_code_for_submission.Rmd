...
if(!exists("mdm2HiLoLMME")){
    ...
    mdm2HiLoLMME <- mdm2HiLoLMME %>% mutate(sig0.05=ifelse(mypadj<0.05 & abs(log2fc) > 0.75,TRUE,FALSE))
...
if(!exists("mdm2LoTreLMME")){
  ...
  mdm2LoTreLMME <- mdm2LoTreLMME %>% mutate(alpha=ifelse(mypadj<0.1 & abs(log2fc) > 0.75,0.05,0.025))
  mdm2LoTreLMME <- mdm2LoTreLMME %>% mutate(size=ifelse(mypadj<0.1 & abs(log2fc) > 0.75,4,2))
...
mdm2LoTreLMME = mdm2LoTreLMME %>%
    mutate(class=ifelse(mypadj<0.05 & abs(log2fc) > 0.75,
                        as.character(metabmeta$SUPER.PATHWAY[match(name,metabmeta$BIOCHEMICAL)]),
                        "Not Significant"))
write.csv(mdm2LoTreLMME,"~/Desktop/temp.csv")
